...
def main():
    target_frame_num = [120,240,850]
    vedio_path = [r"depth.avi" , r"left_depthgray.mp4",r"left.avi"]

    capd = cv2.VideoCapture(vedio_path[0])
    capl = cv2.VideoCapture(vedio_path[2])
    capld = cv2.VideoCapture(vedio_path[1])
        ...
        for target in target_frame_num:
            ...
            for landmark in landmarks:
                if landmark.visibility >= 0.8:
                    ...
                    landmark_x = int(landmark.x * 1280)
                    landmark_y = int(landmark.y * 720)
            ...
            df.to_csv(f"20240312/depout{target}.csv",index = False)
            print(depvalue)
    capd.release()
    capl.release()
    capld.release()
